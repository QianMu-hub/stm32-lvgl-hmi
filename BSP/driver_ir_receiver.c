...
/**
 * @brief  获取系统运行时间：读取 TIM2 当前计数值（计数频率 84MHz）并换算为纳秒
 * @param  无
 * @retval 系统时间，单位: 纳秒(ns)；TIM2 为 32 位计数器，约 51 秒后回绕归零
 */
uint64_t system_get_ns(void)
{
    uint32_t cnt = TIM2->CNT;                // 读取当前计数值
    // 定时器时钟频率 84 MHz → 每个 tick = 1/84e6 秒 ≈ 11.90476 ns
    // 使用 64 位乘法避免溢出
    return (uint64_t)cnt * 1000000000ULL / 84000000ULL;
...
void IRReceiver_Test(void)
{
    ...
    while (1)
    {
        ips200_show_string(0, 0, "IR Receiver: ");        
        ips200_show_string(0, 20, "Device  Data");
				ips200_show_uint(0,80,system_get_ns(),10);
        if (!IRReceiver_Read(&dev, &data))
        {
            ips200_show_uint(150, 0, dev, 10);
            ips200_show_uint(150, 20, data, 10);
            ips200_show_string(0, 60, "Key name: ");
					  ips200_show_string(80, 60, "              ");

            ips200_show_string(80, 60, IRReceiver_CodeToString(data));
        }
    }
}
